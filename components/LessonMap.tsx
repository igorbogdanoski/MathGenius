...
const LessonMap: React.FC<Props> = ({ onEnterTeacherMode }) => {
  ...
  // FOG OF WAR LOGIC
  // Find the index of the first incomplete lesson to determine visibility limit
  let furthestUnlockedIndex = 0;
  if (isLessonCompleted('Start')) {
      furthestUnlockedIndex = 0; // Default to first lesson (11.1)
         ...
         <main className="max-w-7xl mx-auto p-6 pb-32">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
               ...
               {LESSON_MAP.map((lesson, idx) => {
                  ...
                  // Fog Logic: If the lesson is more than 1 step ahead of the furthest unlocked, hide it completely
                  if (idx > furthestUnlockedIndex) {
                      return null; 
                  }
                  
                  // Future/Clouded Node (The one right after the current active one)
                          ...
                          className={`w-full p-5 rounded-2xl border flex items-center gap-5 transition-all text-left group relative ${
                             isLocked 
                               ...
                               : isDone
                                 ...
                                 : isMasterTask
                                    ? 'bg-yellow-50 border-yellow-200 hover:border-yellow-400 shadow-md transform hover:scale-[1.02]'
                       ...
                       >
                          ...
                          <div className="flex-1">
                             <div className="flex items-center gap-2 mb-1">
                                {isMasterTask ? (
                                    <span className="bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider">
                                        {labels.masterTask}
                                    </span>
                                ) : (
                                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                                       isLocked ? 'bg-slate-200 text-slate-500' : isDone ? 'bg-green-200 text-green-800' : 'bg-purple-100 text-purple-700'
                                    }`}>
               ...
               {/* New Unit Placeholder - Only appears after Master Task is done */}
               {isUnitComplete && (
            ...
            {/* Fog of War Overlay for bottom of list if not complete */}
            {!isUnitComplete && (
            ...
            <div className="mt-12 text-center">
              <button 
                onClick={handleLogoClick} // Secret teacher entry
